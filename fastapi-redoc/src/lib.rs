...
//! ```toml
//! [dependencies]
//! fastapi-redoc = { version = "4", features = ["actix-web"] }
//! ```
//!
//! # Using standalone
//!
...
///
/// By default this trait is implemented for [`fastapi::openapi::OpenApi`], [`String`], [`&str`] and
/// [`serde_json::Value`].
///
/// * **OpenApi** implementation allows using fastapi's OpenApi struct as a OpenAPI spec resource
///   for the [`Redoc`].
...
/// # Examples
///
/// _**Use [`Redoc`] to serve fastapi's OpenApi.**_
/// ```no_run
/// # use fastapi_redoc::Redoc;
/// # use fastapi::openapi::OpenApiBuilder;
/// #
...
///
/// The config file must be defined via _**`FASTAPI_REDOC_CONFIG_FILE`**_ env variable for your
...
/// ```rust
/// # fn main() {
/// println!("cargo:rustc-env=FASTAPI_REDOC_CONFIG_FILE=redoc.config.json");
/// # }
/// ```
///
/// _**Defining config file at application startup.**_
/// ```bash
/// FASTAPI_REDOC_CONFIG_FILE=redoc.config.json cargo run
/// ```
///
/// [redoc_config]: <https://redocly.com/docs/api-reference-docs/configuration/functionality/#configuration-options-for-api-docs>
...
impl Config for FileConfig {
    fn load(self) -> Value {
        let path = env::var("FASTAPI_REDOC_CONFIG_FILE")
            .expect("Missing `FASTAPI_REDOC_CONFIG_FILE` env variable, cannot load file config.");
