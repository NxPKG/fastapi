...
impl Nullable {
    ...
    pub fn into_schema_type_token_stream(self) -> proc_macro2::TokenStream {
        if self.0 {
            quote! {fastapi::openapi::schema::Type::Null}
        } else {
...
impl_feature! {
    ...
    ///
    /// This feature supports only syntax parsed from fastapi specific macro attributes, it does not
...
impl As {
    ...
    pub fn to_schema_formatted_string(&self) -> String {
        // See: https://github.com/nxpkg/fastapi/pull/187#issuecomment-1173101405
...
impl ToTokens for AdditionalProperties {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        tokens.extend(quote!(
            fastapi::openapi::schema::AdditionalProperties::FreeForm(
...
impl ToTokens for Discriminator {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let discriminator = if !mapping.is_empty() {
            ...
            quote! {
                fastapi::openapi::schema::Discriminator::with_mapping(#property_name, #mapping)
            }
        } else {
            quote! {
                fastapi::openapi::schema::Discriminator::new(#property_name)
            }
        };
