    ...
    fn to_tokens(&self, tokens: &mut TokenStream) -> Result<(), Diagnostics> {
        ...
        if let Some(Bound(bound)) = variant.get_schema_bound() {
            ...
            for param in self.generics.type_params() {
                ...
                where_clause
                    .predicates
                    .push(parse_quote!(#param : fastapi::ToSchema))
            }
        }

        tokens.extend(quote! {
            impl #impl_generics fastapi::__dev::ComposeSchema for #ident #ty_generics #where_clause {
                fn compose(
                    mut generics: Vec<fastapi::openapi::RefOr<fastapi::openapi::schema::Schema>>
                ) -> fastapi::openapi::RefOr<fastapi::openapi::schema::Schema> {
                    #variant.into()
                }
            }

            impl #impl_generics fastapi::ToSchema for #ident #ty_generics #where_clause {
...
impl UnitStructVariant {
    fn new(root: &Root<'_>) -> Result<Self, Diagnostics> {
        let mut tokens = quote! {
            fastapi::openapi::Object::builder()
...
impl NamedStructSchema {
    pub fn new(
        ...
        let object_tokens = fields_vec
            ...
            .into_iter()
            .fold(
                quote! { let mut object = fastapi::openapi::ObjectBuilder::new(); },
                |mut object_tokens,
                 (
        ...
        let all_of = if !flatten_fields.is_empty() {
            ...
            if flattened_tokens.is_empty() {
                ...
                tokens.extend(quote! {
                    fastapi::openapi::AllOfBuilder::new()
                        #flattened_tokens
        ...
        if !all_of && container_rules.deny_unknown_fields {
            tokens.extend(quote! {
                .additional_properties(Some(fastapi::openapi::schema::AdditionalProperties::FreeForm(false)))
            });
        }
