                ...
                (
                    if schema.is_all_of {
                        let object_builder_tokens =
                            quote! { fastapi::openapi::schema::Object::builder() };
        ...
        if self.untagged {
            tokens.extend(quote! {
                fastapi::openapi::schema::Object::builder()
                    .schema_type(fastapi::openapi::schema::Type::Null)
...
impl<'a> EnumSchema<ObjectSchema> {
    ...
    fn tag(mut self, tag: &'a str, tag_schema: PlainSchema) -> Self {
        ...
        content.0.extend(quote! {
            .property(#tag, fastapi::openapi::schema::Object::builder() #tag_schema)
            .required(#tag)
        });
...
impl EnumSchema<InternallyTaggedUnnamedSchema> {
    fn new<T: ToTokens>(item: T, is_reference: bool) -> Self {
        ...
        let tokens = if is_reference {
            quote! {
                fastapi::openapi::schema::AllOfBuilder::new()
                    .item(#schema)
            }
        } else {
            quote! {
                #schema
                .schema_type(fastapi::openapi::schema::Type::Object)
            }
        };
    ...
    fn new<N: ToTokens>(name: N) -> Self {
        ...
        Self {
            content: Some(PlainSchema(quote! {
                fastapi::openapi::schema::Object::builder() #plain_schema
            })),
            untagged: false,
            features: Vec::new(),
        }
    ...
    T: ToTokens,
{
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        tokens.extend(quote! {
            Into::<fastapi::openapi::schema::OneOfBuilder>::into(fastapi::openapi::OneOf::with_capacity(#len))
                #items_as_tokens
                #discriminator
        });
    }
