...
impl ToTokens for HttpMethod {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        let path_item_type = match self {
            Self::Get => quote! { fastapi::openapi::HttpMethod::Get },
            Self::Post => quote! { fastapi::openapi::HttpMethod::Post },
            Self::Put => quote! { fastapi::openapi::HttpMethod::Put },
            Self::Delete => quote! { fastapi::openapi::HttpMethod::Delete },
            Self::Options => quote! { fastapi::openapi::HttpMethod::Options },
            Self::Head => quote! { fastapi::openapi::HttpMethod::Head },
            Self::Patch => quote! { fastapi::openapi::HttpMethod::Patch },
            Self::Trace => quote! { fastapi::openapi::HttpMethod::Trace },
        };
...
impl<'p> ToTokensDiagnostics for Path<'p> {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) -> Result<(), Diagnostics> {
        ...
        let operation_id = self
            ...
            .ok_or_else(|| {
                Diagnostics::new("operation id is not defined for path")
                    .help(format!(
                        "Try to define it in #[fastapi::path(operation_id = {})]",
                        &fn_name
                    ))
                    .help("Did you define the #[fastapi::path(...)] over function?")
            })?;
        ...
        if methods.is_empty() {
            let diagnostics = || {
                Diagnostics::new("path operation(s) is not defined for path")
                    .help("Did you forget to define it, e.g. #[fastapi::path(get, ...)]")
                    .help("Or perhaps #[fastapi::path(method(head, get), ...)]")
            };
        ...
        let path = self
            ...
            .ok_or_else(|| {
                let diagnostics = || {
                    Diagnostics::new("path is not defined for #[fastapi::path(...)]").help(
                        r#"Did you forget to define it in #[fastapi::path(..., path = "...")]"#,
                    )
                };
        ...
        fn to_schema_references(
            ...
            for reference in component_schema.schema_references {
                ...
                let should_collect_schema = (matches!(
                    ...
                    fastapi_config::SchemaCollect::NonInlined
                ) && !is_inline)
                    || matches!(
                        crate::CONFIG.schema_collect,
                        fastapi_config::SchemaCollect::All
                    );
            ...
            #[cfg(feature = "actix_extras")]
            {
                ...
                if self.path_attr.impl_for.is_none() && !self.ext_methods.is_empty() {
                    ...
                    tokens.extend(quote! {
                        ...
                        impl<'t> fastapi::__dev::Tags<'t> for #fn_ident {
                            fn tags() -> Vec<&'t str> {
                                #path_struct::tags()
                            }
                        }
                        impl fastapi::Path for #fn_ident {
                            fn path() -> String {
                                #path_struct::path()
                            }

                            fn methods() -> Vec<fastapi::openapi::path::HttpMethod> {
                                #path_struct::methods()
                            }

                            fn operation() -> fastapi::openapi::path::Operation {
                                #path_struct::operation()
                            }
                        }

                        impl fastapi::__dev::SchemaReferences for #fn_ident {
                            fn schemas(schemas: &mut Vec<(String, fastapi::openapi::RefOr<fastapi::openapi::schema::Schema>)>) {
                                <#path_struct as fastapi::__dev::SchemaReferences>::schemas(schemas);
                            }
                        }
                    })
                }
        ...
        tokens.extend(quote! {
            impl<'t> fastapi::__dev::Tags<'t> for #impl_for {
                fn tags() -> Vec<&'t str> {
                    #tags_list.into()
                }
            }
            impl fastapi::Path for #impl_for {
                fn path() -> String {
                ...
                fn operation() -> fastapi::openapi::path::Operation {
                    use fastapi::openapi::ToArray;
                    use std::iter::FromIterator;
                    #operation.into()
                }
            }

            impl fastapi::__dev::SchemaReferences for #impl_for {
                fn schemas(schemas: &mut Vec<(String, fastapi::openapi::RefOr<fastapi::openapi::schema::Schema>)>) {
                    #schemas
                    #response_schemas
                }
...
impl ToTokensDiagnostics for Operation<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream2) -> Result<(), Diagnostics> {
        tokens.extend(quote! { fastapi::openapi::path::OperationBuilder::new() });
