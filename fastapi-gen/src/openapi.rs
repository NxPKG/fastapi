...
impl ToTokens for Tag {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let name = &self.name;
        tokens.extend(quote! {
            fastapi::openapi::tag::TagBuilder::new().name(#name)
        });
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let parameters = self
            ...
            .map(|variable| {
                ...
                quote! {
                    .parameter(#name, fastapi::openapi::server::ServerVariableBuilder::new()
        ...
        tokens.extend(quote! {
            fastapi::openapi::server::ServerBuilder::new()
                .url(#url)
                #description
                #parameters
                .build()
                ...
                .map(|item| {
                    ...
                    quote_spanned! {span=>
                        .nest(#path, {
                            ...
                            impl fastapi::__dev::NestedApiConfig for #nest_api_config {
                                fn config() -> (fastapi::openapi::OpenApi, Vec<&'static str>, &'static str) {
                                    let api = <#nest_api as fastapi::OpenApi>::openapi();

                                    (api, #tags.into(), #module_path)
                                }
                            }
                            <#nest_api_config as fastapi::OpenApi>::openapi()
                        })
                    }
                })
                .collect::<TokenStream>();
...
impl ToTokensDiagnostics for OpenApi<'_> {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) -> Result<(), Diagnostics> {
        ...
        let handler_schemas = handlers.iter().fold(
            quote! {
                    let components = openapi.components.get_or_insert(fastapi::openapi::Components::new());
            ...
            |mut handler_schemas, (usage, ..)| {
                handler_schemas.extend(quote! {
                    <#usage as fastapi::__dev::SchemaReferences>::schemas(&mut schemas);
                });
        ...
        let modifiers_tokens = attributes
            ...
            .map(|modifiers| {
                ...
                quote! {
                    let _mods: [&dyn fastapi::Modify; #modifiers_len] = [#modifiers];
        ...
        tokens.extend(quote! {
            impl fastapi::OpenApi for #ident {
                fn openapi() -> fastapi::openapi::OpenApi {
                    use fastapi::{ToSchema, Path};
                    let mut openapi = fastapi::openapi::OpenApiBuilder::new()
                        .info(#info)
                        .paths({
                            #path_items
                        })
...
impl crate::ToTokensDiagnostics for Components {
    fn to_tokens(&self, tokens: &mut TokenStream) -> Result<(), Diagnostics> {
        ...
        let builder_tokens = self
            ...
            .into_iter()
            .fold(
                quote! { fastapi::openapi::ComponentsBuilder::new() },
                |mut components, (component_schema, type_path)| {
                    ...
                    components.extend(quote! { .schemas_from_iter( {
                        let mut schemas = Vec::<(String, fastapi::openapi::RefOr<fastapi::openapi::schema::Schema>)>::new();
                        <#type_path as fastapi::ToSchema>::schemas(&mut schemas);
                        schemas
                    } )});
...
fn impl_paths(handler_paths: Option<&Punctuated<ExprPath, Comma>>) -> Paths {
    ...
    let handlers_impls = handlers
        ...
        .map(|(usage, tag, handler_ident_nested)| {
            quote! {
                ...
                #[allow(non_camel_case_types)]
                impl fastapi::__dev::PathConfig for #handler_ident_nested {
                    fn path() -> String {
                        #usage::path()
                    }
                    ...
                    fn tags_and_operation() -> (Vec<&'static str>, fastapi::openapi::path::Operation) {
                        let item = #usage::operation();
                        let mut tags = <#usage as fastapi::__dev::Tags>::tags();
    ...
    let tokens = handler_paths.into_iter().flatten().fold(
        quote! { #handlers_impls fastapi::openapi::path::PathsBuilder::new() },
        |mut paths, handler| {
            let segments = handler.path.segments.iter().collect::<Vec<_>>();
            let handler_config_name = segments
                .iter()
