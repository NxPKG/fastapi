...
impl TypeTree<'_> {
    ...
    /// Get possible global alias defined in `fastapi_config::Config` for current `TypeTree`.
...
impl ComponentSchema {
                ...
                Some(quote! {
                    {
                        ...
                        fastapi::openapi::schema::SchemaType::from_iter([#schema_type_inner, #nullable_schema_type])
                    }
                })
            } else {
                None
    ...
    fn vec_to_tokens(
        ...
        let schema = quote! {
            fastapi::openapi::schema::ArrayBuilder::new()
                #schema_type
    ...
    fn non_generic_to_tokens(
        ...
        match type_tree.value_type {
            ValueType::Primitive => {
                ...
                tokens.extend(quote! {
                    fastapi::openapi::ObjectBuilder::new().schema_type(#schema_type_tokens)
                });
            ...
            ValueType::Value => {
                ...
                if type_tree.is_value() {
                    tokens.extend(quote! {
                        fastapi::openapi::ObjectBuilder::new()
                            .schema_type(fastapi::openapi::schema::SchemaType::AnyValue)
            ...
            ValueType::Object => {
                ...
                if type_tree.is_object() {
                    ...
                    fn nullable_one_of_item(nullable: bool) -> Option<TokenStream> {
                        if nullable {
                            Some(
                                quote! { .item(fastapi::openapi::schema::ObjectBuilder::new().schema_type(fastapi::openapi::schema::Type::Null)) },
                            )
                        } else {
                            None
                        }
                    ...
                    if let Some(children) = &type_tree.children {
                        ...
                        name_tokens.extend(quote! { std::borrow::Cow::Owned(format!("{}_{}", < #rewritten_path as fastapi::ToSchema >::name(), #children_name)) });
                    } else {
                        name_tokens.extend(
                            quote! { format!("{}", < #rewritten_path as fastapi::ToSchema >::name()) },
                        );
                    }
                    ...
                    if is_inline {
                        ...
                        let items_tokens = if let Some(children) = &type_tree.children {
                            ...
                            if index.is_some() {
                                quote_spanned! {type_path.span()=>
                                    let _ = <#rewritten_path as fastapi::PartialSchema>::schema;

                                    if let Some(composed) = generics.get_mut(#index) {
                                        composed.clone()
                                    } else {
                                        <#rewritten_path as fastapi::PartialSchema>::schema()
                                    }
                                }
                            } else {
                                quote_spanned! {type_path.span()=>
                                    <#rewritten_path as fastapi::__dev::ComposeSchema>::compose(#composed_generics.to_vec())
                                }
                            }
                        } else {
                            quote_spanned! {type_path.span()=>
                        ...
                        object_schema_reference.references =
                            quote! { <#rewritten_path as fastapi::ToSchema>::schemas(schemas) };
                        ...
                        {
                            quote_spanned! {type_path.span()=>
                                fastapi::openapi::schema::OneOfBuilder::new()
                                    #nullable_item
                                    .item(#items_tokens)
                                #title_tokens
                                #default_tokens
                        ...
                        if index.is_none() {
                            let reference_tokens = if let Some(children) = &type_tree.children {
                                ...
                                .collect::<Array<_>>();
                                quote! { <#rewritten_path as fastapi::__dev::ComposeSchema>::compose(#composed_generics.to_vec()) }
                            } else {
                                quote! { <#rewritten_path as fastapi::PartialSchema>::schema() }
                            };
                        ...
                        let schema = if default.is_some() || nullable || title.is_some() {
                            composed_or_ref(quote_spanned! {type_path.span()=>
                                fastapi::openapi::schema::OneOfBuilder::new()
                                    #nullable_item
                                    .item(fastapi::openapi::schema::RefBuilder::new()
                                        #description_stream
                            ...
                            composed_or_ref(quote_spanned! {type_path.span()=>
                                fastapi::openapi::schema::RefBuilder::new()
                                    #description_stream
                                    .ref_location_from_schema_name(#name_tokens)
                            })
                        };
                ...
                type_tree
                    ...
                    .map_try(|children| {
                        let prefix_items = children
                            ...
                            .map(|child| {
                                ...
                                match ComponentSchema::new(ComponentSchemaProps {
                                    ...
                                    Ok(child) => Ok(quote! {
                                        Into::<fastapi::openapi::schema::Schema>::into(#child)
                                    }),
                        ...
                        Result::<TokenStream, Diagnostics>::Ok(quote! {
                            fastapi::openapi::schema::ArrayBuilder::new()
                                #nullable_schema_type
                                .items(fastapi::openapi::schema::ArrayItems::False)
                                .prefix_items(#prefix_items)
                                #description_stream
                                #deprecated
                        })
                    })?
                    .unwrap_or_else(|| quote!(fastapi::openapi::schema::empty())) // TODO should
                    // this bee type "null"?
                    .to_tokens(tokens);
                tokens.extend(features.to_token_stream());
            }
        ...
        let children = children
            ...
            .map(|type_tree| {
                ...
                if let Some(children) = &type_tree.children {
                    ...
                    Ok(quote! { std::borrow::Cow::Owned(format!("{}_{}", <#rewritten_name as fastapi::ToSchema>::name(), #children_name)) })
                } else {
                    Ok(quote! { <#rewritten_name as fastapi::ToSchema>::name() })
                }
            })
        ...
        <I as std::iter::IntoIterator>::IntoIter: 'v,
    {
        let iter = children.into_iter().map(|child| {
            ...
            if let Some(children) = &child.children {
                ...
                Ok(quote! { <#rewritten_path as fastapi::__dev::ComposeSchema>::compose(#items.to_vec()) })
            } else {
                Ok(quote! { <#rewritten_path as fastapi::PartialSchema>::schema() })
            }
        }).collect::<Result<Vec<_>, Diagnostics>>()?
        .into_iter();
        ...
        let iter = children.into_iter().map(|type_tree| {
            if let Some(children) = &type_tree.children {
                ...
                Ok(ChildRefIter::Once(std::iter::once(SchemaReference {
                    name: quote! { String::from(< #rewritten_path as fastapi::ToSchema >::name().as_ref()) },
                    tokens: quote! { <#rewritten_path as fastapi::PartialSchema>::schema() },
                    references: quote !{ <#rewritten_path as fastapi::ToSchema>::schemas(schemas) },
                    is_inline: false,
                    no_recursion: false,
                }))
                )
