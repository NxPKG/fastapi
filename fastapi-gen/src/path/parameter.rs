        ...
        match self {
            ...
            Parameter::IntoParamsIdent(IntoParamsIdentParameter {
                ...
                tokens.extend(quote_spanned! {last_ident.span()=>
                    .parameters(
                        Some(<#path as fastapi::IntoParams>::into_params(#parameter_in_provider))
                    )
                })
            }
        }
...
impl ToTokensDiagnostics for ValueParameter<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) -> Result<(), Diagnostics> {
        let name = &*self.name;
        tokens.extend(quote! {
            fastapi::openapi::path::ParameterBuilder::from(fastapi::openapi::path::Parameter::new(#name))
        });
        ...
        if !schema_features.is_empty() && self.parameter_schema.is_none() {
            return Err(
                ...
                .help("See docs for more details <https://docs.rs/fastapi/latest/fastapi/attr.path.html#parameter-type-attributes>")
            );
        }
...
pub struct IntoParamsIdentParameter<'i> {
    ...
    /// quote!{ ... } of function which should implement `parameter_in_provider` for [`fastapi::IntoParams::into_param`]
...
impl ToTokens for ParameterIn {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.extend(match self {
            Self::Path => quote! { fastapi::openapi::path::ParameterIn::Path },
            Self::Query => quote! { fastapi::openapi::path::ParameterIn::Query },
            Self::Header => quote! { fastapi::openapi::path::ParameterIn::Header },
            Self::Cookie => quote! { fastapi::openapi::path::ParameterIn::Cookie },
        })
    }
}

/// See definitions from `fastapi` crate path.rs
#[derive(Copy, Clone)]
#[cfg_attr(feature = "debug", derive(Debug))]
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            ParameterStyle::Matrix => {
                tokens.extend(quote! { fastapi::openapi::path::ParameterStyle::Matrix })
            }
            ParameterStyle::Label => {
                tokens.extend(quote! { fastapi::openapi::path::ParameterStyle::Label })
            }
            ParameterStyle::Form => {
                tokens.extend(quote! { fastapi::openapi::path::ParameterStyle::Form })
            }
            ParameterStyle::Simple => {
                tokens.extend(quote! { fastapi::openapi::path::ParameterStyle::Simple })
            }
            ParameterStyle::SpaceDelimited => {
                tokens.extend(quote! { fastapi::openapi::path::ParameterStyle::SpaceDelimited })
            }
            ParameterStyle::PipeDelimited => {
                tokens.extend(quote! { fastapi::openapi::path::ParameterStyle::PipeDelimited })
            }
            ParameterStyle::DeepObject => {
                tokens.extend(quote! { fastapi::openapi::path::ParameterStyle::DeepObject })
            }
        }
    }
}
