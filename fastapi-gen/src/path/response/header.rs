...
/// Parsed representation of response header defined in `#[fastapi::path(..)]` attribute.
///
...
impl ToTokensDiagnostics for Header {
    fn to_tokens(&self, tokens: &mut TokenStream) -> Result<(), Diagnostics> {
        if let Some(header_type) = &self.value_type {
            ...
            tokens.extend(quote! {
                fastapi::openapi::HeaderBuilder::new().schema(#media_type_schema)
            })
        } else {
            // default header (string type)
            tokens.extend(quote! {
                Into::<fastapi::openapi::HeaderBuilder>::into(fastapi::openapi::Header::default())
            })
        };
