...
impl ToTokensDiagnostics for ToResponse<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) -> Result<(), Diagnostics> {
        ...
        tokens.extend(quote! {
            impl #to_response_impl_generics fastapi::ToResponse <#lifetime> for #ident #ty_generics #where_clause {
                fn response() -> (& #lifetime str, fastapi::openapi::RefOr<fastapi::openapi::response::Response>) {
                    (#name, #response.into())
                }
            }
        });
...
impl ToTokensDiagnostics for IntoResponses {
    fn to_tokens(&self, tokens: &mut TokenStream) -> Result<(), Diagnostics> {
        let responses = match &self.data {
            ...
            Data::Enum(enum_value) => enum_value
                ...
                .map(|response| {
                    let status = &response.status_code;
                    let response_tokens = as_tokens_or_diagnostics!(response);
                    Ok(quote!((#status, fastapi::openapi::RefOr::from(#response_tokens))))
                })
        ...
        tokens.extend(quote!{
                impl #impl_generics fastapi::IntoResponses for #ident #ty_generics #where_clause {
                    fn responses() -> std::collections::BTreeMap<String, fastapi::openapi::RefOr<fastapi::openapi::response::Response>> {
                        fastapi::openapi::response::ResponsesBuilder::new()
                            #responses
                            .build()
                            .into()
                    }
