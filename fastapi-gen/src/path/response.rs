...
pub enum Response<'r> {
    /// A type that implements `fastapi::IntoResponses`.
...
/// Parsed representation of response attributes from `#[fastapi::path]` attribute.
#[derive(Default)]
#[cfg_attr(feature = "debug", derive(Debug))]
    ...
    fn to_tokens(&self, tokens: &mut TokenStream2) -> Result<(), Diagnostics> {
        match self.inner.as_ref() {
            Some(ResponseTupleInner::Ref(res)) => {
                ...
                if res.is_inline {
                    tokens.extend(quote_spanned! {path.span()=>
                        <#path as fastapi::ToResponse>::response().1
                    });
                } else {
                    tokens.extend(quote! {
                        fastapi::openapi::Ref::from_response_name(<#path as fastapi::ToResponse>::response().0)
                    });
                }
            }
            Some(ResponseTupleInner::Value(value)) => {
                ...
                tokens.extend(quote! {
                    fastapi::openapi::ResponseBuilder::new().description(#description)
                });
            ...
            None => tokens.extend(quote! {
                fastapi::openapi::ResponseBuilder::new().description("")
            }),
        }
    ...
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) -> Result<(), Diagnostics> {
        tokens.extend(
            self.0
                ...
                .into_iter()
                .fold(
                    quote! { fastapi::openapi::ResponsesBuilder::new() },
                    |mut acc, response| {
