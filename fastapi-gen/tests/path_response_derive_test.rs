...
use fastapi::openapi::{RefOr, Response};
use fastapi::{OpenApi, Path, ToResponse};
...
macro_rules! test_fn {
    ( module: $name:ident, responses: $($responses:tt)* ) => {
        ...
        mod $name {
            ...
            #[fastapi::path(get,path = "/foo",responses $($responses)*)]
            fn get_foo() {}
        }
    };
}
...
fn derive_path_with_multiple_responses_via_content_attribute_auto_collect_responses() {
    #[derive(serde::Serialize, fastapi::ToSchema)]
    #[allow(unused)]
    struct User {
        id: String,
    }
...
fn path_response_with_inline_ref_type() {
    #[derive(serde::Serialize, fastapi::ToSchema, fastapi::ToResponse)]
    #[allow(unused)]
    struct User {
        name: String,
    }
...
fn path_response_with_no_schema() {
    ...
    /// Post some secret inner handler
    #[fastapi::path(post, path = "/api/inner/secret", responses(
