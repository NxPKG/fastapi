...
fn derive_name_struct_response() {
    ...
    let (name, v) = <Person as fastapi::ToResponse>::response();
...
fn derive_enum_response() {
    ...
    let (name, v) = <PersonType as fastapi::ToResponse>::response();
    let value = serde_json::to_value(v).unwrap();
...
fn derive_response_with_unit_type() {
    ...
    let (name, v) = <PersonSuccessResponse as fastapi::ToResponse>::response();
    let value = serde_json::to_value(v).unwrap();
    ...
    ( $(#[$meta:meta])* $key:ident $ident:ident $($tt:tt)* ) => {
        {
            ...
            let responses = <$ident as fastapi::IntoResponses>::responses();
