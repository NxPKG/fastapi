...
use fastapi::openapi::path::ParameterBuilder;
use fastapi::{IntoParams, OpenApi, Path, ToSchema};
use fastapi_gen::schema;
...
fn path_with_enum_path_param() {
    ...
    impl IntoParams for ApiVersion {
        fn into_params(
            _: impl Fn() -> Option<fastapi::openapi::path::ParameterIn>,
        ) -> Vec<fastapi::openapi::path::Parameter> {
            vec![ParameterBuilder::new()
                .description(Some(""))
                .name("api_version")
                .required(fastapi::openapi::Required::True)
                .parameter_in(fastapi::openapi::path::ParameterIn::Path)
                .schema(Some(schema!(
                    #[inline]
                    ApiVersion
                )))
