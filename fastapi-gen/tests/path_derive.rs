...
macro_rules! api_fn_doc_with_params {
    ( $method:ident: $path:literal => $( #[$attr:meta] )* $key:ident $name:ident $body:tt ) => {{
        #[allow(dead_code)]
        #[derive(serde::Deserialize, fastapi::IntoParams)]
        $(#[$attr])*
        $key $name $body
...
fn derive_path_with_datetime_format_query_parameter() {
    #[derive(serde::Deserialize, fastapi::ToSchema)]
    struct Since {
        /// Some date
        #[allow(dead_code)]
        date: String,
...
fn derive_path_with_into_responses() {
    ...
    #[fastapi::path(get, path = "foo", responses(MyResponse))]
    #[allow(unused)]
    fn get_foo() {}
...
fn derive_into_params_required() {
    ...
    #[fastapi::path(get, path = "/params", params(Params))]
    #[allow(unused)]
    fn get_params() {}
    let operation = test_api_fn_doc! {
        get_params,
...
fn path_and_nest_with_additional_tags() {
    mod test_path {
        #[allow(dead_code)]
        #[fastapi::path(get, path = "/test", tag = "this_is_tag", tags = ["additional"])]
        #[allow(unused)]
        fn test_path() -> &'static str {
            ""
        }
    ...
    mod test_nest {
        ...
        #[allow(dead_code)]
        #[fastapi::path(get, path = "/test", tag = "this_is_tag:nest", tags = ["additional:nest"])]
        #[allow(unused)]
        fn test_path_nested() -> &'static str {
            ""
        }
