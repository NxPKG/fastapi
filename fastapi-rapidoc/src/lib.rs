...
//! ```toml
//! [dependencies]
//! fastapi-rapidoc = { version = "4", features = ["actix-web"] }
//! ```
//!
//! # Using standalone
//!
...
pub struct RapiDoc {
    ...
    openapi: Option<fastapi::openapi::OpenApi>,
}
...
impl RapiDoc {
    ...
    pub fn with_openapi<U: Into<Cow<'static, str>>>(
        spec_url: U,
        openapi: fastapi::openapi::OpenApi,
    ) -> Self {
        Self {
            path: Cow::Borrowed(""),
            spec_url: spec_url.into(),
...
mod rocket {
    ...
    #[derive(Clone)]
    struct OpenApiHandler(fastapi::openapi::OpenApi);
