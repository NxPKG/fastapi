...
//! be like utopic? Don't worry fastapi is just there to fill this gap. It aims to do if not all then
...
//!
//! Fastapi crate provides autogenerated OpenAPI documentation for Rust REST APIs. It treats
...
//!
//! Currently fastapi provides simple integration with actix-web framework but is not limited to the actix-web
...
//! |Flavor|Support|
//! |--|--|
//! |[actix-web](https://github.com/actix/actix-web)|Parse path, path parameters and query parameters, recognize request body and response body, [`fastapi-actix-web` bindings](https://docs.rs/fastapi-actix-web). See more at [docs][actix_path]|
//! |[axum](https://github.com/tokio-rs/axum)|Parse path and query parameters, recognize request body and response body, [`fastapi-axum` bindings](https://docs.rs/fastapi-axum). See more at [docs][axum_path]|
...
//!
//! Refer to the existing [examples](https://github.com/nxpkg/fastapi/tree/master/examples) to find out more.
//!
//! ## Features
//!
//! * OpenAPI 3.1
...
//! * Rust type aliases via [`fastapi-config`][fastapi_config].
//!
...
//!   [examples](https://github.com/nxpkg/fastapi/tree/master/examples) for more details.
...
//!   or [examples](https://github.com/nxpkg/fastapi/tree/master/examples) for more details
...
//!   See the [`request_body`](https://docs.rs/fastapi/latest/fastapi/openapi/request_body/index.html) and
//!   [`response`](https://docs.rs/fastapi/latest/fastapi/openapi/response/index.html) docs for examples.
...
//! * **`config`** Enables [`fastapi-config`](https://docs.rs/fastapi-config/) for the project which allows
//!   defining global configuration options for `fastapi`.
//!
...
//! # Examples
//!
//! _**Create type with `ToSchema` and use it in `#[fastapi::path(...)]` that is registered to the `OpenApi`.**_
//!
//! ```rust
...
//!
//! * See how to serve OpenAPI doc via Swagger UI check [`fastapi-swagger-ui`][fastapi_swagger] crate for more details.
...
//! [fastapi_swagger]: https://docs.rs/fastapi-swagger-ui/
//! [fastapi_config]: https://docs.rs/fastapi-config/
//!
...
/// struct OpenApiDoc;
///
/// impl fastapi::OpenApi for OpenApiDoc {
///     fn openapi() -> fastapi::openapi::OpenApi {
///         use fastapi::{ToSchema, Path};
///         fastapi::openapi::OpenApiBuilder::new()
///             .info(fastapi::openapi::InfoBuilder::new()
///                 .title("application name")
///                 .version("version")
///                 .description(Some("application description"))
///                 .license(Some(fastapi::openapi::License::new("MIT")))
///                 .contact(
///                     Some(fastapi::openapi::ContactBuilder::new()
///                         .name(Some("author name"))
///                         .email(Some("author email")).build()),
///             ).build())
///             .paths(fastapi::openapi::path::Paths::new())
///             .components(Some(fastapi::openapi::Components::new()))
///             .build()
///     }
/// }
/// ```
...
pub trait ToSchema: PartialSchema {
    ...
    /// # impl<T: ToSchema> fastapi::PartialSchema for Foo<T> {
    /// #     fn schema() -> fastapi::openapi::RefOr<fastapi::openapi::schema::Schema> {
    /// #         Default::default()
    /// #     }
    /// # }
    ...
    /// Implement reference [`fastapi::openapi::schema::Schema`]s for this type.
    ///
    /// When [`ToSchema`] is being derived this is implemented automatically but if one needs to
    /// manually implement [`ToSchema`] trait then this is needed for `fastapi` to know
    ...
    ///
    /// ```rust
    /// # use fastapi::{ToSchema, PartialSchema};
    /// #
    /// #[derive(ToSchema)]
    /// struct Owner {
    ///     name: String
...
// Create `fastapi` module so we can use `fastapi-gen` directly from `fastapi` crate.
...
/// # use fastapi::openapi::schema::{SchemaType, KnownFormat, SchemaFormat, ObjectBuilder, Schema};
...
/// ```rust
/// # use fastapi::PartialSchema;
/// # use fastapi::openapi::schema::{Type, KnownFormat, SchemaFormat, ObjectBuilder, Schema};
/// # use fastapi::openapi::RefOr;
/// #
...
/// ```rust
/// # use fastapi::PartialSchema;
/// # use fastapi::openapi::schema::ObjectBuilder;
/// struct Pet {
///     id: i32,
///     name: String,
/// }
...
///
/// This trait is implemented via [`#[fastapi::path(...)]`][derive] attribute macro and there
/// is no need to implement this trait manually.
///
/// # Examples
///
/// Use `#[fastapi::path(..)]` to implement Path trait
/// ```rust
...
/// ```
///
/// Example of what would manual implementation roughly look like of above `#[fastapi::path(...)]` macro.
/// ```rust
/// fastapi::openapi::PathsBuilder::new().path(
///         "/pets/{id}",
///         fastapi::openapi::PathItem::new(
///             fastapi::openapi::HttpMethod::Get,
///             fastapi::openapi::path::OperationBuilder::new()
///                 .responses(
///                     fastapi::openapi::ResponsesBuilder::new()
///                         .response(
///                             "200",
///                             fastapi::openapi::ResponseBuilder::new()
///                                 .description("Pet found successfully")
///                                 .content("application/json",
///                                     fastapi::openapi::Content::new(
///                                         Some(fastapi::openapi::Ref::from_schema_name("Pet")),
///                                     ),
///                             ),
///                         )
///                         .response("404", fastapi::openapi::Response::new("Pet was not found")),
///                 )
///                 .operation_id(Some("get_pet_by_id"))
///                 .deprecated(Some(fastapi::openapi::Deprecated::False))
///                 .summary(Some("Get pet by id"))
///                 .description(Some("Get pet by id\n\nGet pet from database by pet database id\n"))
///                 .parameter(
///                     fastapi::openapi::path::ParameterBuilder::new()
///                         .name("id")
...
///                         .schema(
///                             Some(fastapi::openapi::ObjectBuilder::new()
///                                 .schema_type(fastapi::openapi::schema::Type::Integer)
///                                 .format(Some(fastapi::openapi::SchemaFormat::KnownFormat(fastapi::openapi::KnownFormat::Int64)))),
///                         ),
///                 )
///                 .tag("pet_api"),
///         ),
...
/// *(Before `fastapi::OpenApi::openapi()` function returns)*.
...
/// ```rust
/// # use fastapi::{OpenApi, Modify};
/// # use fastapi::openapi::security::{SecurityScheme, HttpBuilder, HttpAuthScheme};
/// #[derive(OpenApi)]
...
pub trait Modify {
    ...
    /// [`fastapi::OpenApi`] before it is served.
...
/// #    name: String,
/// # }
/// impl fastapi::IntoParams for PetParams {
///     fn into_params(
///         parameter_in_provider: impl Fn() -> Option<fastapi::openapi::path::ParameterIn>
...
pub trait IntoParams {
    /// Provide [`Vec`] of [`openapi::path::Parameter`]s to caller. The result is used in `fastapi-gen` library to
...
/// ```rust
/// # use fastapi::Number;
/// # use fastapi::openapi::schema::{ObjectBuilder, SchemaType, Type};
...
/// Internal dev module used internally by fastapi-gen
#[doc(hidden)]
#[cfg(feature = "macros")]
#[cfg_attr(doc_cfg, doc(cfg(feature = "macros")))]
pub mod __dev {
    ...
    use crate::{fastapi, OpenApi, PartialSchema};
    ...
    impl<T: PathConfig> fastapi::Path for T {
        fn path() -> String {
    ...
    pub trait ComposeSchema {
        fn compose(
            new_generics: Vec<fastapi::openapi::RefOr<fastapi::openapi::schema::Schema>>,
        ) -> fastapi::openapi::RefOr<fastapi::openapi::schema::Schema>;
    }
    ...
    macro_rules! impl_compose_schema {
        ( $( $ty:ident ),* ) => {
            ...
            impl ComposeSchema for $ty {
                fn compose(_: Vec<fastapi::openapi::RefOr<fastapi::openapi::schema::Schema>>) -> fastapi::openapi::RefOr<fastapi::openapi::schema::Schema> {
                    schema!( $ty ).into()
                }
            }
            )*
    ...
    fn schema_or_compose<T: ComposeSchema>(
        schemas: Vec<fastapi::openapi::RefOr<fastapi::openapi::schema::Schema>>,
        index: usize,
    ) -> fastapi::openapi::RefOr<fastapi::openapi::schema::Schema> {
    ...
    impl<T: ComposeSchema> ComposeSchema for Option<T> {
        fn compose(
            ...
            fastapi::openapi::schema::OneOfBuilder::new()
                .item(
                    fastapi::openapi::schema::ObjectBuilder::new()
